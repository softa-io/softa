...
public class GetDataNode implements NodeProcessor<GetDataParams> {
    ...
    private Object executeSingleRow(GetDataParams nodeParams, Filters filters) {
        ...
        Optional<Map<String, Object>> optionalMap = modelService.searchOne(nodeParams.getModelName(), flexQuery);
        if (ONE_FIELD_VALUE.equals(nodeParams.getGetDataType()) && optionalMap.isPresent()) {
            ...
            return optionalMap.get().get(fieldName);
        } else {
            return optionalMap.orElse(null);
        }
    }
