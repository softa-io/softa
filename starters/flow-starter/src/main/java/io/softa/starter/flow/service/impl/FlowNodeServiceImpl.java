...
import io.softa.starter.flow.node.NodeFactory;
import io.softa.starter.flow.node.params.LoopByDatasetParams;
...
public class FlowNodeServiceImpl extends EntityServiceImpl<FlowNode, Long> implements FlowNodeService {
    ...
    private LoopByDatasetParams extractLoopByDatasetParams(FlowNode flowNode) {
        ...
        try {
            loopByDatasetParams = JsonUtils.jsonNodeToObject(flowNode.getNodeParams(), LoopByDatasetParams.class);
    ...
    private LoopByPageParams extractLoopByPageParams(FlowNode flowNode) {
        ...
        try {
            loopByPageParams = JsonUtils.jsonNodeToObject(flowNode.getNodeParams(), LoopByPageParams.class);
        } catch (JSONException e) {
    ...
    private void executeLoopByPage(FlowNode flowNode, NodeContext nodeContext) {
        ...
        long totalCount = modelService.count(loopByPageParams.getModel(), clonedFilters);
        if (totalCount == 0) {
            return;
        }
