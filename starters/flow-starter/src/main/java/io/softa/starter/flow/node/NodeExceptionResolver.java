...
import io.softa.framework.base.exception.FlowException;
...
public class NodeExceptionResolver {
    ...
    private static NodeExceptionPolicy getNodeExceptionPolicy(FlowNode flowNode) {
        ...
        try {
            exceptionPolicy = JsonUtils.jsonNodeToObject(flowNode.getExceptionPolicy(), NodeExceptionPolicy.class);
    ...
    private static boolean checkResultForException(Object value, NodeExceptionPolicy exceptionPolicy) {
        return switch (exceptionPolicy.getExceptionWhen()) {
            case RESULT_IS_EMPTY -> checkResultIsEmpty(value);
            case RESULT_IS_EMPTY_OR_ZERO -> checkResultIsEmpty(value) || value.equals(0) || value.equals(0L);
            case RESULT_IS_NOT_EMPTY -> !checkResultIsEmpty(value);
            case RESULT_IS_FALSE -> value == null || Boolean.FALSE.equals(value);
            case RESULT_IS_TRUE -> Boolean.TRUE.equals(value);
        };
    }
