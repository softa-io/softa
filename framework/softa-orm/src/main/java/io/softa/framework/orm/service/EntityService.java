 ...
 *
 * @param <T> the entity type, extending {@link AbstractModel}
 * @param <K> the type of the entity's primary key, must be {@link Serializable}, commonly Long or String.
 */
public interface EntityService<T extends AbstractModel, K extends Serializable> {
     ...
     */
    void createOrUpdate(List<T> entities, List<SFunction<T, ?>> uniqueConstraints);
     ...
     */
    void createOrUpdate(List<T> entities, boolean ignoreNull);
     ...
     * @param ignoreNull whether to ignore null values
     */
    void createOrUpdate(List<T> entities, List<SFunction<T, ?>> uniqueConstraints, boolean ignoreNull);
     ...
     * @return an Optional object containing the matching entity, or empty if not found
     */
    Optional<T> searchOne(FlexQuery flexQuery);
