 ...
 * @param <K> id class
 */
public abstract class EntityServiceImpl<T extends AbstractModel, K extends Serializable> implements EntityService<T, K> {
     ...
     */
    @Override
    public boolean exist(Filters filters) {
    ...
    /**
     * Creates or updates entities by their ID.
     * Ignore null values by default.
     *
     * @param entities the list of entities to create or update
     */
    @Override
    public void createOrUpdate(List<T> entities) {
        this.createOrUpdate(entities, true);
    }

    /**
     * Creates or updates entities by unique constraint fields.
     * Ignore null values by default.
     *
     * @param entities the list of entities to create or update
     ...
     */
    @Override
    public void createOrUpdate(List<T> entities, List<SFunction<T, ?>> uniqueConstraints) {
        this.createOrUpdate(entities, uniqueConstraints, true);
    }
     ...
     */
    @Override
    public void createOrUpdate(List<T> entities, boolean ignoreNull) {
        List<Map<String, Object>> rows = BeanTool.objectsToMapList(entities, ignoreNull);
        modelService.createOrUpdate(modelName, rows, List.of(ModelConstant.ID));
    }
     ...
     */
    @Override
    public void createOrUpdate(List<T> entities, List<SFunction<T, ?>> uniqueConstraints, boolean ignoreNull) {
        List<Map<String, Object>> rows = BeanTool.objectsToMapList(entities, ignoreNull);
        List<String> uniqueFields = uniqueConstraints.stream()
                .map(LambdaUtils::getAttributeName)
                .collect(Collectors.toList());
        modelService.createOrUpdate(modelName, rows, uniqueFields);
    }
     ...
     * updating the fields specified in the object, ignore null values.
     ...
     * @return an Optional object containing the matching entity, or empty if not found
     */
    @Override
    public Optional<T> searchOne(Filters filters) {
     ...
     */
    @Override
    public Optional<T> searchOne(FlexQuery flexQuery) {
        Optional<Map<String, Object>> optionalMap = modelService.searchOne(modelName, flexQuery);
        return optionalMap.map(row -> BeanTool.mapToObject(row, entityClass));
    }
    ...
    public Map<Serializable, T> groupById(Filters filters) {
        ...
        if (!CollectionUtils.isEmpty(objects)) {
            return objects.stream().collect(Collectors.toMap(AbstractModel::getId, Function.identity()));
        }
