...
public class DataSourceByModelAspect {
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        // If model-level datasource is not configured or equals current, proceed directly
        if (StringUtils.isBlank(modelDataSourceKey) || modelDataSourceKey.equals(previousDataSource)) {
            return joinPoint.proceed();
        }
        boolean isTransactionActive = TransactionSynchronizationManager.isActualTransactionActive();
        if (isTransactionActive && StringUtils.isNotBlank(previousDataSource)) {
            // In transaction, cannot switch datasource, throw an exception.
            throw new RuntimeException("""
                    Cannot switch to different data source in transactional operation.
                    Current data source: %s, trying to switch to: %s.
                    """.formatted(previousDataSource, modelDataSourceKey));
        }
        // Switch to model-level datasource and proceed
        return DataSourceHolder.callWithDataSource(modelDataSourceKey, joinPoint::proceed);
    }
