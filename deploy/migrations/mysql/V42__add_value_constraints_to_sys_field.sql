...
ALTER TABLE sys_field
    ADD COLUMN min VARCHAR(40) COMMENT 'Smallest accepted value, as a decimal literal; numeric fields only' AFTER scale,
    ADD COLUMN max VARCHAR(40) COMMENT 'Largest accepted value, as a decimal literal; numeric fields only' AFTER min,
    ADD COLUMN pattern VARCHAR(256) COMMENT 'Regex the whole value must match; STRING and TEXT only' AFTER max,
    ADD COLUMN constraint_message VARCHAR(256) COMMENT 'Shown when a bound or the pattern rejects a value; its own i18n key' AFTER pattern;
